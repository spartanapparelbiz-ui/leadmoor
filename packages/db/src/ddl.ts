/**
 * The schema, as one SQL string.
 *
 * This lives in a TypeScript module rather than a .sql file read at runtime. Locating a sibling
 * file through `import.meta.url` works under Node but not once a bundler has rewritten the module,
 * which made `migrate()` fail in the production web build with an opaque path error. A string
 * constant has no such dependency and is the same text under every runtime.
 *
 * Every statement is `IF NOT EXISTS`, so applying it repeatedly is a no-op.
 */
export const DDL = `-- LeadMoor schema DDL. Applied by \`pnpm db:push\`. Idempotent.
